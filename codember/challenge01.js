...
// ====> NO TIENE BUENAS PRATICAS  <=====
function decrypt(text) {
  const arrText = text.split(' ').map((item) => item.toLowerCase());
  const uniqueArr = [...new Set(arrText)]; // Set => solo permite valores únicos
  const myDecrypt = [];
  uniqueArr.forEach((item) => {
    ...
    arrText.forEach((text) => {
      if (item === text) {
        let lengthArr = myDecrypt.length - 1;
        myDecrypt[lengthArr] = item + i;
        i++;
      }
    });
  });

  return myDecrypt.join('');
}
const textResolve =
...
// console.log(decrypt('gato perro perro coche Gato peRRo sol'));
// console.log(decrypt('llaveS casa CASA casa llaves'));
// console.log(decrypt('taza ta za taza'));
// console.log(decrypt('casas casa casasas'));

console.log(decrypt(textResolve));
